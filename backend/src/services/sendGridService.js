...
export const sendVerificationEmail = async (email, code, username) => {
  try {
    ...
    const msg = {
      to: email,
      from: {
        email: process.env.SENDGRID_FROM_EMAIL || 'noreply@worktracker.com',
      ...
      html: `
        ...
        <head>
          ...
          <style>
            ...
            .button { display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: #ffffff; text-decoration: none; padding: 12px 30px; border-radius: 8px; font-weight: 600; margin: 20px 0; }
    ...
    console.log('✅ SendGrid verification email sent:', result[0].statusCode);
...
export const sendPasswordResetEmail = async (email, newPassword, username) => {
  try {
    ...
    const result = await sgMail.send(msg);
    console.log('✅ SendGrid password reset email sent:', result[0].statusCode);
